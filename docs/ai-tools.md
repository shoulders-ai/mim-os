...
- Google read tools: `gmail_inbox`, `gmail_search`, `calendar_events`, `drive_search`, `docs_read`, `sheets_read`.
- Google outbound tools: `gmail_send`, `calendar_create`.

Slack send is a kernel/CLI tool only, not a chat tool. Google outbound chat tools are `external` main-process tool calls and require approval in normal and strict modes (their `high` risk only drives the approval card's caution styling).
...
- `web_read` calls `web.read` and returns cleaned content from HTML/plain-text pages or selectable PDFs. HTML pages are extracted via Mozilla Readability plus the shared HTML-to-Markdown parser; PDFs are extracted locally with `pdfjs-dist` and return text plus `format: "pdf"`, `pages`, and metadata when available. Returns `title`, `content`, `excerpt`, `byline`, `siteName`, `truncated`, and `length`. Only http/https URLs are allowed; private/loopback addresses are blocked. Category `network`, risk `medium`.
- `web_read_auto` calls `web.readAuto` and is the default browser read path for dynamic websites. It tries the rendered Chromium reader first, classifies clear blockers, falls back to the persistent Research Browser profile when the domain is already configured, and can return a recent workspace cache entry after a live blocker. It returns the rendered/read fields plus `source`, `source_domain`, `status`, `attention_required`, `reason`, `attempts`, `capture`, and optional `cache`; `status: "partial"` means the best captured content is returned with readiness/confidence signals and should be inspected or retried with a larger `timeout_ms`, not treated as a user-interruption condition. If a source needs setup, it returns `source_not_configured` with `setup_url` rather than a generic failure. Cached fallback returns `source: "cache"` with `cache.cached_at`, while source health still records the live blocker. Category `network`, risk `medium`.
- `web_read_rendered` calls `web.readRendered` and renders the URL in a hidden Chromium window before extraction. It waits adaptively for page load, DOM/content stability, fonts, and images; captures visible DOM, open shadow roots, and same-origin iframe bodies; converts the captured HTML through the shared parser; and returns `url`, `final_url`, `title`, `content`, `truncated`, `length`, `next_start_char`, `capture`, and extraction `stats`. Capture evidence includes status, confidence, reason, elapsed time, timeout/stability flags, visible text length, and visible link/button/form/table/heading/image counts. It retries one known transient Chromium navigation/capture failure before surfacing an error; readiness timeouts return partial evidence rather than hard failure. Pass `start_from_char` to continue a truncated read, `timeout_ms` for slow SPAs, and `extract_links`/`extract_images` when source URLs matter. Only http/https URLs are allowed; private/loopback addresses are blocked. Category `network`, risk `medium`. Electron desktop registers the Chromium renderer; headless/runtime environments without Chromium report the tool as unavailable.
- `web_read_research` calls `web.readResearch` and reads through the persistent Research Browser profile (`persist:mim-research`) after a domain has been granted with `web.research.allowDomain`. It uses saved cookies, localStorage, IndexedDB, and service-worker state from the visible setup window (`web.research.open`) so agents can work asynchronously against sources the user has already authorized. It returns the same markdown/chunk fields as `web_read_rendered` plus `source`, `allowed_domain`, `status`, `attention_required`, and optional `reason`. Status values classify weak pages such as `consent_required`, `login_required`, `captcha_required`, `security_verification`, `site_error`, and `empty_capture`; the tool does not bypass access controls. Category `network`, risk `medium`.
- `web_research_status` calls `web.research.status` and returns workspace Research Browser state from `.mim/settings.json`: `allowedDomains[]` is the grant list; `sources[]` is the durable source-health list with `domain`, `allowed`, `status` (`ready`, `needs_attention`, `not_configured`), `attentionRequired`, last read URL/status/source timestamps, failure count, and reason. `web.readAuto` and `web.readResearch` update `sources[]` after reads. Kernel-only setup tools `web.research.allowDomain`, `web.research.removeDomain`, `web.research.open`, and `web.research.clearProfile` maintain grants and the persistent profile.

Implementation: `src/main/web/webSearch.ts`, `src/main/web/readUrl.ts`, `src/main/web/readAutoUrl.ts`, `src/main/web/webReadCache.ts`, `src/main/web/readRenderedUrl.ts`, `src/main/web/readResearchUrl.ts`, `src/main/web/researchBrowser.ts`, `src/main/web/researchSettings.ts`, `src/main/web/renderedBrowser.ts`, `src/main/web/renderedCapture.ts`. Tests: `src/main/web/webSearch.test.ts`, `src/main/web/readUrl.test.ts`, `src/main/web/readAutoUrl.test.ts`, `src/main/web/webReadCache.test.ts`, `src/main/web/webReadEvaluation.test.ts`, `src/main/web/readRenderedUrl.test.ts`, `src/main/web/readResearchUrl.test.ts`, `src/main/web/researchBrowser.test.ts`, `src/main/web/researchSettings.test.ts`, `src/main/web/renderedCapture.test.ts`, `src/main/tools/web.test.ts`.
