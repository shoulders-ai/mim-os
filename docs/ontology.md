...
| **Tool** | A single ability an agent or app can call: read a file, search the web, send a message. Every tool call passes the approval policy and lands in the trace. | A skill; an app; an integration (a Connection *provides* tools). | UI "tool" · registry `namespace.verb` |
