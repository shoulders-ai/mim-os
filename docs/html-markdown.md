...
- `src/main/web/readAutoUrl.ts` chooses between the rendered and Research Browser readers, then returns the parser output from whichever path succeeds; if both live paths need attention, it can return a recent complete workspace cache entry without re-running this parser.
...
- `src/main/web/readResearchUrl.ts` uses the same rendered conversion path through a persistent Research Browser profile and adds blocked-page classification.
