...
describe('closeGuardDecision', () => {
  ...
  it('prompts for active package runs only', () => {
    const result = closeGuardDecision(0, 2)
    expect(result.shouldPrompt).toBe(true)
    expect(result.message).toBe('You have 2 active package runs. Quit anyway?')
  })

  it('prompts for a single active package run with singular noun', () => {
    const result = closeGuardDecision(0, 1)
    expect(result.shouldPrompt).toBe(true)
    expect(result.message).toBe('You have 1 active package run. Quit anyway?')
  })

  it('combines dirty tabs and active runs in the message', () => {
    const result = closeGuardDecision(2, 3)
    expect(result.shouldPrompt).toBe(true)
    expect(result.message).toBe('You have 2 unsaved tabs and 3 active package runs. Quit anyway?')
  })
  ...
  it('combines active runs and agent sessions', () => {
    ...
    expect(result.shouldPrompt).toBe(true)
    expect(result.message).toBe('You have 3 active package runs and 2 running agent sessions. Quit anyway?')
  })

  it('combines all three parts with comma and "and"', () => {
    const result = closeGuardDecision(2, 3, 1)
    expect(result.shouldPrompt).toBe(true)
    expect(result.message).toBe('You have 2 unsaved tabs, 3 active package runs and 1 running agent session. Quit anyway?')
  })
  ...
  it('two-arg call shape still works (backward compat)', () => {
    ...
    expect(result.shouldPrompt).toBe(true)
    expect(result.message).toBe('You have 1 unsaved tab and 2 active package runs. Quit anyway?')
  })
})
