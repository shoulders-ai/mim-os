...
describe('preparePtyShellIntegration', () => {
  ...
  it('generates a zsh startup proxy and points scratch zsh at it when enabled', () => {
    ...
    expect(zshrc).toContain('zle -N mim-insert-newline')
    expect(zshrc).toContain("$'\\e[13;2u' mim-insert-newline")
    expect(zshrc).toContain("$'\\e[D' backward-char")
    expect(zshrc).toContain("$'\\e[C' forward-char")
    expect(zshrc).toContain("$'\\eOD' backward-char")
    expect(zshrc).toContain("$'\\eOC' forward-char")
    expect(zshrc).toContain("'^?' backward-delete-char")
    expect(zshrc).toContain("'^H' backward-delete-char")
    expect(zshrc).toContain("$'\\e[3~' delete-char")
  ...
  itIfZsh('keeps xterm Option arrows and fallback control bytes working in zsh vi insert mode', async () => {
    ...
    try {
      ...
      pty.write('line two')
      pty.write('\r')
      await waitForOutput(() => output.includes('ACCEPTED:line one\r\nline two'))

      pty.write('remove-empty-line')
      pty.write('\x1b[13;2u')
      pty.write('\x7f')
      pty.write('\r')
      await waitForOutput(() => output.includes('ACCEPTED:remove-empty-line'))
      ...
      pty.write('X')
      pty.write('\r')
      await waitForOutput(() => output.includes('ACCEPTED:abX\r\ncd'))
      ...
      pty.write('X')
      pty.write('\r')
      await waitForOutput(() => output.includes('ACCEPTED:ef\r\nXgh'))
      ...
      pty.write('\x7f')
      pty.write('\r')
      await waitForOutput(() => output.includes('ACCEPTED:ijkl'))
      ...
      pty.write('\x1b[3~')
      pty.write('\r')
      await waitForOutput(() => output.includes('ACCEPTED:mnop'))
