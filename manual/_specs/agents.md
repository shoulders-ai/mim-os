...
- Steering: model picker and effort control (verify exact UI labels); cost tracking
  per chat; context donut, compaction divider, @mentions, and context chips in
  the composer (verify).
...
- Routines: standing workspace prompts in the Navigator. Explain Run opens the live
  chat transcript, Resume/Pause control local automatic triggers, routine files
  live under `routines/`, local state under `.mim/routines/`, and routine tool
  grants are separate from ordinary chat approvals.
...
- Claude Code, Codex, and Gemini CLI can reach Mim itself over the MCP bridge —
  the same tools, the same approvals. Pi currently runs without Mim tools (one
  paragraph; setup detail → /develop).
